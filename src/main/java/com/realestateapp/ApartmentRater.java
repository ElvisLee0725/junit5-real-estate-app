...
public class ApartmentRater {
	...
	public static int rateApartment(Apartment apartment) {
		...
		// .compareTo() returns -1 if its value is smaller than input. Return 1 if greater than input. 0 means equal
	...
	public static double calculateAverageRating(List<Apartment> apartments) {
		...
		for(Apartment apartment : apartments) {
			sumOfRatings += rateApartment(apartment);
		}

		return sumOfRatings * 1.0 / apartments.size();
	}
}
